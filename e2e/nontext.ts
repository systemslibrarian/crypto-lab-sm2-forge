 ...
 *
 * SM2 Forge defines `--control-border` for inputs, selects, pane controls and
 * buttons, then overrides fills for primary, danger and active-pane states.
 * This oracle judges each live control as painted rather than trusting those
 * tokens, including hover, disabled-gate and failure states reached by the
 * driver.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
    * A driven pass measures controls across all three gated panes and their
    * expanded output states. Sibling controls share the same ancestors, so
    * computed style and geometry are cached for one non-mutating pass.
     */
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // This lab's pane buttons paint all four sides today; the per-side
      // walk is what keeps that an implementation detail rather than a
      // load-bearing assumption.
      //
      // A side also has to be opaque enough to paint. A transparent border is
      // only layout space, never a delineator, so it is excluded here.

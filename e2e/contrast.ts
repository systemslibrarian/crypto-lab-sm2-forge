...
/**
 * Composite-aware WCAG 1.4.3 contrast measurement for SM2 Forge.
 *
 ...
 *    unknown backdrop. SM2 Forge uses `color-mix()` for its pass, failure and
 *    alarm verdicts, warning logs, identity/cipher component washes, hero aside,
 *    and shared top-bar ink. Those surfaces carry the lab's central results,
 *    so a violations-only gate would leave too much undecided.
 *
 *  - TEXT FADED BY AN ANCESTOR'S `opacity` — axe reads the declared `color`,
 *    not always the colour that lands on screen. Disabled gated-pane controls
 *    are exempt as inactive components; every active state is composited here.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. This page wraps
 *    long field values instead of scrolling them, but the guard keeps a future
 *    clipped region from producing a contrast ratio for absent pixels.
 *
...
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     *
    * A driven pass walks the curve parameters, eight Z_A inputs, ciphertext
    * components, two order cards and two attack signatures. Their siblings
    * repeatedly share ancestors up to `<body>`, so style and geometry reads
    * are cached for the duration of one non-mutating pass.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // that paints nothing. `checkVisibility()` catches it. SM2 Forge's
      // signature equation ships shut and the gate opens it through its real
      // `<summary>` control before scanning the expanded state.
      ...
      //
      // This lab does not use the idiom today: each verdict, retirement notice
      // and persistent failure log is visibly rendered. The guard stays for a
      // future visually-hidden announcer and remains deliberately narrow.
     ...
     *
    * The painted `aria-hidden` text here is the verdict glyph beside each
    * written PASS/FAIL/ALARM label and the layout separators between named
    * ciphertext components. The shared header's SVG marks carry no text.
    * `scan()` still measures the whole hidden set with the exemption lifted.
     *
